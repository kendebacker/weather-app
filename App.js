...
export default function KensApp() {
  ...
  const refreshCities = (add) => {
    ...
    Promise.all(
      cities.map((item) =>
        fetch(
          `http://api.openweathermap.org/geo/1.0/direct?q=${item.city},${item.state},${item.country}&limit=10&appid=${API_KEY}`,
        ),
      ),
    )
      .then((responseList) =>
            ...
            res
              ...
              .then((jsonCity) =>
                jsonCity.length === 0
                  ...
                  : fetch(
                      `https://api.openweathermap.org/data/2.5/weather?lat=${jsonCity[0].lat}&lon=${jsonCity[0].lon}&appid=${API_KEY}`,
                    ),
              ),
          ),
        ),
      ...
      .then((responseList) =>
        Promise.all(
          responseList.map((res) =>
            res !== null ? res.json() : Promise.resolve(null),
          ),
        ),
      )
      .then((jsonList) =>
        jsonList.map((json, index) => {
          return {
            ...
            temperature:
              json === null
                ? "??°F"
                : `${Math.round((json.main.temp - 273) * 1.8 + 32, 2)}°F`,
            icon:
              json === null
                ? "cloud-question"
                : icons[json.weather[0].main] === undefined
                ? "cloud-question"
                : icons[json.weather[0].main],
          };
        }),
      )
    ...
    setTime(
      ...
      }/${currentDate.getDate()}/${currentDate.getFullYear()} ${hours}:${minutes}:${seconds} ${amPM}`,
    );
    setCityList(cities);
